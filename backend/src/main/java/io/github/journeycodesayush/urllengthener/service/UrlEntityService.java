...
public class UrlEntityService {
  ...
  public UrlEntityService(
      UrlEntityRepository urlEntityRepository, @Value("${app.backup.version}") int backupVersion) {
  ...
  public UrlBackup exportUrlBackup() {
    List<Url> urls =
        urlEntityRepository.findAll().stream()
            .map(
                (entity) -> {
                  Url url = new Url();
